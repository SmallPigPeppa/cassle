...
class Conv3x3_mofied(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, groups=1, dilation=1, use_expand=False):
        super(Conv3x3_mofied, self).__init__()
        self.conv2d = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                                padding=dilation, groups=groups, bias=False, dilation=dilation)
        self.expand = conv1x1(in_planes, out_planes, stride=1)
    ...
    def forward(self, x):
        if not self.use_expand:
            ...
            with torch.no_grad():
                out1 = self.conv2d(x)
            return self.expand(x) + out1

    def set_expand(self,use_expand=True):
        self.use_expand=use_expand
